...
Features:
  ...
  - Tickbox selection of which analyses to run
  - Live estimated runtime display
...
# Registry: key → (display name, estimated seconds for ~5k points per pop)
ANALYSIS_REGISTRY = {
    "spatial_overlay":   ("01 — Spatial Overlay",                    5),
    "nearest_neighbour": ("02 — Nearest-Neighbour Distance",       10),
    "grid_correlation":  ("03 — Grid-Based Intensity Correlation",   5),
    "proximity_network": ("04 — Proximity Network",                 25),
    "kmeans":            ("05 — K-Means Co-Clustering",             15),
    "dbscan":            ("06 — DBSCAN Density Clustering",         10),
    "ripleys":           ("07 — Ripley's Cross-K / Cross-L",        45),
    "kde_overlap":       ("08 — KDE Overlap",                       15),
    "voronoi":           ("09 — Voronoi Neighbourhood",             10),
}
...
def plot_spatial_overlay(df_a, df_b, name_a, name_b, extent, outdir, run_id):
    ...
    print(f"  ✓ Spatial overlay saved. ({elapsed:.1f}s)")
    return elapsed
...
def nearest_neighbour_analysis(df_a, df_b, name_a, name_b, extent, outdir,
    ...
    plt.close(fig)
    elapsed = time.time() - t0
    print(f"  ✓ Nearest-neighbour analysis saved. ({elapsed:.1f}s)")
...
def grid_correlation_analysis(df_a, df_b, name_a, name_b, extent, outdir,
    ...
    axes[3].scatter(ha, hb, s=20, c="grey", alpha=0.5, edgecolors="k",
                    linewidths=0.2)
    axes[3].set_xlabel(f"Counts ({name_a})")
    axes[3].set_ylabel(f"Counts ({name_b})")
    ...
    plt.close(fig)
    elapsed = time.time() - t0
    print(f"  ✓ Grid correlation analysis saved. ({elapsed:.1f}s)")
    return stats, elapsed
...
def proximity_network_analysis(df_a, df_b, name_a, name_b, extent, outdir,
    ...
    for k, thresh in enumerate(distance_thresholds):
        ...
        if edge_seg:
            norm_e = Normalize(vmin=0, vmax=thresh)
            lc = LineCollection(edge_seg, cmap="YlOrRd_r", norm=norm_e,
                                linewidths=0.4, alpha=0.35)
            lc.set_array(np.array(edge_w))
            ax.add_collection(lc)

        vmax_deg = max(max(deg_a), max(deg_b), 1)
        sc_a = ax.scatter(ca[:, 0], ca[:, 1], c=deg_a, cmap="Reds", s=20,
                          vmin=0, vmax=vmax_deg, edgecolors="k",
        ...
        ax.set_title(f"Thresh={thresh:.1f}\nEdges={n_edges} Comp={n_comp}\n"
                     f"Conn: {name_a}={frac_a:.0%} {name_b}={frac_b:.0%}",
                     fontsize=10)
        legend_el = [
            Line2D([0], [0], marker='o', color='w',
                   markerfacecolor=COLOR_A, markersize=8, label=name_a),
            Line2D([0], [0], marker='s', color='w',
                   markerfacecolor=COLOR_B, markersize=8, label=name_b),
        ]
    ...
    plt.close(fig2)
    elapsed = time.time() - t0
    print(f"  ✓ Proximity network analysis saved. ({elapsed:.1f}s)")
    return stats, elapsed
...
def kmeans_analysis(df_a, df_b, name_a, name_b, extent, outdir, run_id,
    ...
    axes[1, 0].bar(list(k_range), mixing_scores, color=COLOR_BOTH,
                   alpha=0.7, edgecolor="k")
    axes[1, 0].set_xlabel("k"); axes[1, 0].set_ylabel("Mean Mixing Entropy")
    axes[1, 0].set_title("A/B Mixing (1=perfect)")
    ...
    for c in range(best_k):
        ...
        ax.scatter(all_pts[mask_a, 0], all_pts[mask_a, 1], c=[col],
                   marker="o", s=18, alpha=0.6, edgecolors="k",
                   linewidths=0.2)
        ax.scatter(all_pts[mask_b, 0], all_pts[mask_b, 1], c=[col],
                   marker="s", s=18, alpha=0.6, edgecolors="k",
                   linewidths=0.2)
    ...
    fig.tight_layout()
    fig.savefig(outdir / stamped_filename("05_kmeans", run_id),
                bbox_inches="tight")
    plt.close(fig)
...
def dbscan_analysis(df_a, df_b, name_a, name_b, extent, outdir, run_id,
    ...
    for idx, c in enumerate(unique_cl):
        mask = cl == c
        axes[0].scatter(all_pts[mask, 0], all_pts[mask, 1],
                        c=[cmap_db(idx)], s=18, alpha=0.7, edgecolors="k",
                        linewidths=0.2,
                        label=f"C{c}" if idx < 10 else None)
    noise_m = cl == -1
    axes[0].scatter(all_pts[noise_m, 0], all_pts[noise_m, 1],
    ...
    for idx, c in enumerate(unique_cl):
        mask = cl == c
        is_mixed = ((labels_true[mask] == 0).any() and
                     (labels_true[mask] == 1).any())
    ...
    fig.tight_layout()
    fig.savefig(outdir / stamped_filename("06_dbscan", run_id),
                bbox_inches="tight")
    plt.close(fig)
    elapsed = time.time() - t0
    print(f"  ✓ DBSCAN analysis saved. ({elapsed:.1f}s)")
    return stats, elapsed
...
def ripleys_cross_k(df_a, df_b, name_a, name_b, extent, outdir, run_id,
    ...
    plt.close(fig)
    elapsed = time.time() - t0
    print(f"  ✓ Ripley's cross-K/L saved. ({elapsed:.1f}s)")
    return stats, elapsed
...
def kde_overlap_analysis(df_a, df_b, name_a, name_b, extent, outdir, run_id,
    ...
    za_n = Za / max(Za.max(), 1e-15)
    zb_n = Zb / max(Zb.max(), 1e-15)
    ...
    plt.close(fig)
    elapsed = time.time() - t0
    print(f"  ✓ KDE overlap saved. ({elapsed:.1f}s)")
    return stats, elapsed
...
def voronoi_analysis(df_a, df_b, name_a, name_b, extent, outdir, run_id):
    ...
    n = len(all_pts)
    if n < 4:
        print("  ⚠ Too few points for Voronoi; skipping.")
        return {}, 0.0
    ...
    stats = {
        ...
        f"Voronoi segregation ({name_a})":
            1 - mc_a / exp_a if exp_a else np.nan,
        f"Voronoi segregation ({name_b})":
            1 - mc_b / exp_b if exp_b else np.nan,
    }
    ...
    sc = ax.scatter(all_pts[:, 0], all_pts[:, 1], c=cross_frac,
                    cmap="RdYlGn", s=25, edgecolors="k", linewidths=0.3,
    ...
    axes[1].hist(cross_frac[:len(ca)], bins=bins_f, color=COLOR_A,
                 alpha=0.6, label=name_a, edgecolor="k", linewidth=0.3)
    axes[1].hist(cross_frac[len(ca):], bins=bins_f, color=COLOR_B,
                 alpha=0.6, label=name_b, edgecolor="k", linewidth=0.3)
    ...
    axes[1].set_xlabel("Cross-type fraction")
    axes[1].set_ylabel("Count")
    axes[1].set_title("Cross-Type Distribution")
    ...
    plt.close(fig)
    elapsed = time.time() - t0
    print(f"  ✓ Voronoi analysis saved. ({elapsed:.1f}s)")
    return stats, elapsed
...
def save_summary(all_stats: dict, outdir: Path, run_id: str,
    ...
    if timings:
        for key, secs in timings.items():
            label = ANALYSIS_REGISTRY.get(key, (key,))[0]
            rows.append({"Section": "Timing", "Metric": label,
                         "Value": f"{secs:.1f}s"})
        rows.append({"Section": "Timing", "Metric": "TOTAL",
                      "Value": f"{sum(timings.values()):.1f}s"})
    ...
    if timings:
        ...
        print(f"   TOTAL: {format_duration(total)} ({total:.1f}s)")
...
def run_analysis(csv_a: str, csv_b: str, name_a: str, name_b: str,
                 outdir: str, selected_analyses: set = None):
    """Run colocalization pipeline — only the ticked analyses."""
    if selected_analyses is None:
        selected_analyses = set(ANALYSIS_REGISTRY.keys())

    run_id = generate_run_id()
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)

    n_selected = len(selected_analyses)
    n_total = len(ANALYSIS_REGISTRY)
    est_total = sum(ANALYSIS_REGISTRY[k][1] for k in selected_analyses)
    ...
    print(f"  Analyses   : {n_selected}/{n_total} selected")
    print(f"  Est. time  : {format_duration(est_total)}")
    print(f"{'=' * 70}\n")

    for key, (label, est) in ANALYSIS_REGISTRY.items():
        marker = "  ✓" if key in selected_analyses else "  ✗"
        t_str = f" ({format_duration(est)})" if key in selected_analyses else ""
        print(f"  {marker}  {label}{t_str}")
    print()
    ...
    all_stats = {}
    timings = {}
    common = (df_a, df_b, name_a, name_b, extent, outdir, run_id)

    t_total_start = time.time()
    print("Running selected analyses...\n")

    if "spatial_overlay" in selected_analyses:
        elapsed = plot_spatial_overlay(*common)
        timings["spatial_overlay"] = elapsed

    if "nearest_neighbour" in selected_analyses:
        stats, elapsed = nearest_neighbour_analysis(*common)
        all_stats["Nearest-Neighbour"] = stats
        timings["nearest_neighbour"] = elapsed

    if "grid_correlation" in selected_analyses:
        stats, elapsed = grid_correlation_analysis(*common)
        all_stats["Grid Correlation"] = stats
        timings["grid_correlation"] = elapsed

    if "proximity_network" in selected_analyses:
        stats, elapsed = proximity_network_analysis(*common)
        all_stats["Proximity Network"] = stats
        timings["proximity_network"] = elapsed

    if "kmeans" in selected_analyses:
        stats, elapsed = kmeans_analysis(*common)
        all_stats["K-Means"] = stats
        timings["kmeans"] = elapsed

    if "dbscan" in selected_analyses:
        stats, elapsed = dbscan_analysis(*common)
        all_stats["DBSCAN"] = stats
        timings["dbscan"] = elapsed

    if "ripleys" in selected_analyses:
        stats, elapsed = ripleys_cross_k(*common)
        all_stats["Ripley's Cross-K"] = stats
        timings["ripleys"] = elapsed

    if "kde_overlap" in selected_analyses:
        stats, elapsed = kde_overlap_analysis(*common)
        all_stats["KDE Overlap"] = stats
        timings["kde_overlap"] = elapsed

    if "voronoi" in selected_analyses:
        stats, elapsed = voronoi_analysis(*common)
        all_stats["Voronoi"] = stats
        timings["voronoi"] = elapsed

    t_total = time.time() - t_total_start

    if all_stats or timings:
        save_summary(all_stats, outdir, run_id, timings)
    else:
        print("\n  (No statistical analyses selected — no summary CSV.)")

    print(f"\n  Total wall time: {format_duration(t_total)} ({t_total:.1f}s)")
    print(f"  Estimated was:   {format_duration(est_total)}")

    return run_id, timings
...
# ──────────────────────────────────────────────────────────────────────────────
def launch_gui():
    """Launch Tkinter GUI with analysis checkboxes and runtime estimates."""
    ...
    # Analysis checkbox variables — all ON by default
    analysis_vars = {}
    ...
    subtitle = ttk.Label(
        main_frame,
        text="Select two CSV files, tick the analyses you want, "
             "then press Run.\n"
    ...
    def browse_csv(var):
        path = filedialog.askopenfilename(
            ...
            filetypes=[("CSV files", "*.csv *.tsv *.txt"),
                       ("All files", "*.*")])
        if path:
            var.set(path)

    ttk.Label(file_frame, text="CSV File A:").grid(
        row=0, column=0, sticky="w", padx=(0, 5), pady=3)
    ttk.Entry(file_frame, textvariable=var_csv_a, width=55).grid(
        row=0, column=1, padx=5, pady=3)
    ttk.Button(file_frame, text="Browse…",
               command=lambda: browse_csv(var_csv_a)).grid(
        row=0, column=2, padx=5, pady=3)

    ttk.Label(file_frame, text="CSV File B:").grid(
        row=1, column=0, sticky="w", padx=(0, 5), pady=3)
    ttk.Entry(file_frame, textvariable=var_csv_b, width=55).grid(
    ...
    label_frame = ttk.LabelFrame(main_frame, text="  Population Labels  ",
                                  padding=10)
    label_frame.grid(row=3, column=0, columnspan=3, sticky="ew",
                     pady=(0, 8))

    ttk.Label(label_frame, text="Name for A:").grid(
        row=0, column=0, sticky="w", padx=(0, 5), pady=3)
    ttk.Entry(label_frame, textvariable=var_name_a, width=30).grid(
        row=0, column=1, padx=5, pady=3, sticky="w")

    ttk.Label(label_frame, text="Name for B:").grid(
        row=1, column=0, sticky="w", padx=(0, 5), pady=3)
    ttk.Entry(label_frame, textvariable=var_name_b, width=30).grid(
        row=1, column=1, padx=5, pady=3, sticky="w")

    # ── Analysis Selection with Runtime Estimates ──
    analysis_frame = ttk.LabelFrame(
        main_frame, text="  Select Analyses to Run  ", padding=10)
    analysis_frame.grid(row=4, column=0, columnspan=3, sticky="ew",
                        pady=(0, 8))

    # Top row: buttons + summary
    btn_sel_frame = ttk.Frame(analysis_frame)
    btn_sel_frame.grid(row=0, column=0, columnspan=3, sticky="ew",
                       pady=(0, 8))

    def select_all():
        for v in analysis_vars.values():
            v.set(True)
    ...
    ttk.Button(btn_sel_frame, text="Select All",
               command=select_all).grid(row=0, column=0, padx=(0, 5))
    ttk.Button(btn_sel_frame, text="Deselect All",
               command=deselect_all).grid(row=0, column=1, padx=(0, 15))

    # Count + estimated time label
    var_estimate_lbl = tk.StringVar(value="")
    estimate_lbl = ttk.Label(btn_sel_frame, textvariable=var_estimate_lbl,
                              font=("Helvetica", 10, "bold"),
                              foreground="#1a5276")
    estimate_lbl.grid(row=0, column=2, padx=(5, 0), sticky="w")

    def update_estimate(*_):
        n_sel = sum(v.get() for v in analysis_vars.values())
        total_sec = sum(
            ANALYSIS_REGISTRY[k][1]
            for k, v in analysis_vars.items() if v.get()
        )
        if n_sel == 0:
            var_estimate_lbl.set("No analyses selected")
        else:
            var_estimate_lbl.set(
                f"{n_sel}/{len(ANALYSIS_REGISTRY)} selected  •  "
                f"Estimated runtime: {format_duration(total_sec)}"
            )

    # Checkbox grid: 2 columns, each with name + time estimate
    keys = list(ANALYSIS_REGISTRY.keys())
    n_per_col = (len(keys) + 1) // 2

    for idx, key in enumerate(keys):
        label, est_sec = ANALYSIS_REGISTRY[key]
        col = 0 if idx < n_per_col else 1
        row = (idx % n_per_col) + 1

        # Frame for each checkbox row to hold checkbox + time label
        cb_frame = ttk.Frame(analysis_frame)
        cb_frame.grid(row=row, column=col, sticky="w", padx=(5, 10),
                      pady=1)

        cb = ttk.Checkbutton(cb_frame, text=label,
                              variable=analysis_vars[key],
                              command=update_estimate)
        cb.grid(row=0, column=0, sticky="w")

        time_lbl = ttk.Label(cb_frame,
                              text=f"({format_duration(est_sec)})",
                              font=("Helvetica", 8),
                              foreground="#888888")
        time_lbl.grid(row=0, column=1, sticky="w", padx=(6, 0))

    # Trace all vars
    for v in analysis_vars.values():
        v.trace_add("write", update_estimate)
    update_estimate()

    # ── Estimate note ──
    note_lbl = ttk.Label(
        analysis_frame,
        text="⏱  Times are rough estimates for ~5k points per population. "
             "Larger datasets will take longer.",
        font=("Helvetica", 8, "italic"), foreground="#999999")
    note_lbl.grid(row=n_per_col + 2, column=0, columnspan=2, sticky="w",
                  pady=(8, 0))

    # ── Output Directory ──
    out_frame = ttk.LabelFrame(main_frame, text="  Output Directory  ",
                                padding=10)
    ...
    ttk.Label(out_frame, text="Directory:").grid(
        row=1, column=0, sticky="w", padx=(0, 5), pady=3)
    ttk.Entry(out_frame, textvariable=var_outdir, width=55).grid(
        row=1, column=1, padx=5, pady=3)
    ...
    def validate_inputs():
        if not var_csv_a.get().strip():
            messagebox.showerror("Missing Input",
                                 "Please select CSV File A.")
            return False
        if not var_csv_b.get().strip():
            messagebox.showerror("Missing Input",
                                 "Please select CSV File B.")
            return False
        ...
        n_sel = sum(v.get() for v in analysis_vars.values())
        if n_sel == 0:
            messagebox.showwarning("No Analyses Selected",
                                   "Please tick at least one analysis "
                                   "to run.")
            return False
        return True
    ...
    def run_in_thread():
        ...
        selected = {key for key, var in analysis_vars.items() if var.get()}
        n_sel = len(selected)
        est_sec = sum(ANALYSIS_REGISTRY[k][1] for k in selected)

        btn_run.configure(state="disabled")
        progress.start(15)
        var_status.set(
            f"⏳ Running {n_sel} analyses "
            f"(est. {format_duration(est_sec)}) — please wait...")

        def worker():
            try:
                rid, timings = run_analysis(
                    csv_a, csv_b, name_a, name_b, outdir, selected)
                actual = sum(timings.values()) if timings else 0
                root.after(0, lambda: on_success(
                    rid, outdir, n_sel, est_sec, actual))
    ...
    def on_success(rid, outdir, n_sel, est_sec, actual_sec):
        progress.stop()
        btn_run.configure(state="normal")
        var_status.set(
            f"✅ Done!  Run ID: {rid}  •  "
            f"{n_sel} analyses  •  "
            f"Actual: {format_duration(actual_sec)} "
            f"(est. was {format_duration(est_sec)})")
        messagebox.showinfo(
            "Analysis Complete",
            f"All outputs saved to:\n{Path(outdir).resolve()}\n\n"
            f"Run ID: {rid}\n"
            f"Analyses run: {n_sel}/{len(ANALYSIS_REGISTRY)}\n"
            f"Actual time: {format_duration(actual_sec)}\n"
            f"Estimated was: {format_duration(est_sec)}\n\n"
    ...
    footer.grid(row=8, column=0, columnspan=3, pady=(15, 0))
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and not sys.argv[1].startswith("--gui"):
        ...
        selected = set(args.analyses) if args.analyses else None
        run_analysis(args.csv_a, args.csv_b, args.name_a, args.name_b,
                     outdir, selected)
    else:
        launch_gui()