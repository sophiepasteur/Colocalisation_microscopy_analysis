...
# Registry of all available analyses (key → display name)
ANALYSIS_REGISTRY = {
    "spatial_overlay":    "01 — Spatial Overlay",
    "nearest_neighbour":  "02 — Nearest-Neighbour Distance",
    "grid_correlation":   "03 — Grid-Based Intensity Correlation",
    "proximity_network":  "04 — Proximity Network",
    "kmeans":             "05 — K-Means Co-Clustering",
    "dbscan":             "06 — DBSCAN Density Clustering",
    "ripleys":            "07 — Ripley's Cross-K / Cross-L",
    "kde_overlap":        "08 — KDE Overlap",
    "voronoi":            "09 — Voronoi Neighbourhood",
}
...
# ──────────────────────────────────────────────────────────────────────────────
# MAIN PIPELINE  (now accepts selected_analyses set)
# ──────────────────────────────────────────────────────────────────────────────
def run_analysis(csv_a: str, csv_b: str, name_a: str, name_b: str,
    ...
    # List what will run
    for key, label in ANALYSIS_REGISTRY.items():
    ...
    if "spatial_overlay" in selected_analyses:
        plot_spatial_overlay(*common)

    if "nearest_neighbour" in selected_analyses:
        all_stats["Nearest-Neighbour"] = nearest_neighbour_analysis(*common)

    if "grid_correlation" in selected_analyses:
        all_stats["Grid Correlation"] = grid_correlation_analysis(*common)

    if "proximity_network" in selected_analyses:
        all_stats["Proximity Network"] = proximity_network_analysis(*common)

    if "kmeans" in selected_analyses:
        all_stats["K-Means"] = kmeans_analysis(*common)

    if "dbscan" in selected_analyses:
        all_stats["DBSCAN"] = dbscan_analysis(*common)

    if "ripleys" in selected_analyses:
        all_stats["Ripley's Cross-K"] = ripleys_cross_k(*common)

    if "kde_overlap" in selected_analyses:
        all_stats["KDE Overlap"] = kde_overlap_analysis(*common)

    if "voronoi" in selected_analyses:
        all_stats["Voronoi"] = voronoi_analysis(*common)
...
# ──────────────────────────────────────────────────────────────────────────────
def launch_gui():
    """Launch Tkinter GUI with analysis checkboxes."""
    ...
    btn_sel_frame.grid(row=0, column=0, columnspan=3, sticky="w",
                       pady=(0, 5))
    ...
    ttk.Button(btn_sel_frame, text="Deselect All",
               command=deselect_all).grid(row=0, column=1, padx=(0, 5))

    # Count label (updates dynamically)
    var_count_lbl = tk.StringVar(value="")

    def update_count(*_):
        n = sum(v.get() for v in analysis_vars.values())
        var_count_lbl.set(f"{n}/{len(ANALYSIS_REGISTRY)} selected")

    count_lbl = ttk.Label(btn_sel_frame, textvariable=var_count_lbl,
                           font=("Helvetica", 9, "italic"),
                           foreground="#666666")
    count_lbl.grid(row=0, column=2, padx=(15, 0))

    # Checkbox grid (2 columns)
    keys = list(ANALYSIS_REGISTRY.keys())
    n_per_col = (len(keys) + 1) // 2  # split into 2 columns

    for idx, key in enumerate(keys):
        col = 0 if idx < n_per_col else 1
        row = (idx % n_per_col) + 1  # +1 for button row
        cb = ttk.Checkbutton(analysis_frame,
                              text=ANALYSIS_REGISTRY[key],
                              variable=analysis_vars[key],
                              command=update_count)
        cb.grid(row=row, column=col, sticky="w", padx=(5, 20), pady=2)

    # Trace all vars to update count
    for v in analysis_vars.values():
    ...
    def validate_inputs():
        ...
        if not var_outdir.get().strip():
            ...
            return False
        # Check at least one analysis is selected
    ...
    def run_in_thread():
        ...
        progress.start(15)
        var_status.set(f"⏳ Running {n_sel} analyses — please wait...")

        def worker():
            try:
                rid = run_analysis(csv_a, csv_b, name_a, name_b, outdir,
                                   selected)
                root.after(0, lambda: on_success(rid, outdir, n_sel))
    ...
    def on_success(rid, outdir, n_sel):
        progress.stop()
        btn_run.configure(state="normal")
        var_status.set(f"✅ Done!  Run ID: {rid}  "
                       f"({n_sel} analyses completed)")
        messagebox.showinfo(
            "Analysis Complete",
    ...
    if len(sys.argv) > 1 and not sys.argv[1].startswith("--gui"):
        ...
        parser.add_argument(
            ...
            help="Which analyses to run (default: all). "
                 f"Options: {', '.join(ANALYSIS_REGISTRY.keys())}")
